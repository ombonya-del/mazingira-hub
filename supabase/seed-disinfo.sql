-- Adds "fuller picture" fields to disinfo_items and seeds the current Narrative
-- Disinfo desk so every card gets the full "on raia" rebuttal. Run ONCE in
-- Supabase → SQL Editor. (To re-seed: delete from public.disinfo_items where published = true; first.)

alter table public.disinfo_items add column if not exists source_url   text;  -- "fuller picture" article link
alter table public.disinfo_items add column if not exists source_label text;  -- e.g. "Citizen Digital · Magadi suspension"
alter table public.disinfo_items add column if not exists video_id     text;  -- YouTube id for a "fuller picture" clip

insert into public.disinfo_items
  (name, handle, posted_at, body, eng, verdict, verdict_class, tweet_url, source_url, source_label, video_id, rebuttal, published, created_at) values
...
('Circulating on X', null, null,
 ...
 'https://citizen.digital/article/cs-joho-suspends-operations-for-tata-chemicals-magadi-mining-over-non-compliance-n387332',
 'Citizen Digital · Tata Chemicals Magadi suspension', null,
...
('Circulating on X', null, null,
 ...
 'amplifying in reply threads · this week', 'False dilemma', 'vc', null,
 'https://www.standardmedia.co.ke/environment-climate/article/2001551935/government-bows-to-pressure-on-airstrip-plan-convenes-public-participation',
 'The Standard · Imenti airstrip · public participation', null,
...
('Circulating on X', null, null,
 ...
 'https://citizen.digital/article/public-participation-exercise-begins-for-ksh500b-siaya-nuclear-power-plant-n386871',
 'Citizen Digital · Siaya public participation', null,
...
('Circulating on X', null, null,
 ...
 null, 'NTV Kenya · Lamu', 'UtI7OMaJh7M',
...
('Circulating on TikTok', null, null,
 ...
 null, 'Olkaria geothermal', 'vMSGSoJ3L6M',
