/* MazingiraKenya · Raia — service worker
   ...
   Note: raia-privacy.html is a real static page — it is served directly, never via
   the app shell (see the navigate handler's denylist). */
const CACHE = "raia-v1";
const SHELL = [
  "./", "./raia.html", "./raia.webmanifest",
  "./icon-192.png", "./icon-512.png"
];
const DENYLIST = [/raia-privacy\.html$/];
...
self.addEventListener("fetch", function(e){
  ...
  if(req.mode === "navigate"){
    e.respondWith(
      caches.match("./raia.html").then(function(cached){
        var net = fetch(req).then(function(r){
          var copy = r.clone(); caches.open(CACHE).then(function(c){ c.put("./raia.html", copy); });
          return r;
